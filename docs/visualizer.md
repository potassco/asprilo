...
You can load an instance that contains a plan like default instances. It can be found on the right side of the main window.
...
Your encoding should contain `#show init/2.` and `#show occurs/3.` statements to pipe correct. It also accepts input in asp format. See [Instance format](#Instance format) for more information.
E.g:
...
- `init(object([object type], [object ID]), value(at, ([X], [Y]))).`: Creates an object of the type 'object type' with the ID 'object ID' at the position ('X', 'Y'). The following object types are possible: 'robot', 'pickingStation', 'chargingStation', 'shelf', 'node', 'highway'. If you use one of the other types there must always be an object of the 'node' type at the same position.
...
- `init(object(robot, [robot ID]), value(energy_cost, ([action], [energy cost]))).`: Specifies the energy costs for a robot with the ID 'robot ID' for a specific action.
...
- `occurs(object(robot, [RobotID]), action(pickup, ()), [time step]).`: The robot with the id 'RobotID' picks up the self on the same position as the robot at the time step 'time step'.
