...
-If you use `occurs(object(robot, [RobotID]), action(deliver, ([order ID], [product ID])), [time step]).` instead of `occurs(object(robot, [RobotID]), action(deliver, ([order ID],[product ID],[product amount])), [time step]).` the visualizer can`t track the amount of removed products from shelves.
